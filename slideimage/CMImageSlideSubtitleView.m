...
#define CM_SubtitleLabelframe CGRectMake(10.0f, 10.0f, CM_ScreenFrame.size.width- 30.0f, 30.0f)
#define CM_CountLabelframe CGRectMake(CM_ScreenFrame.size.width- 60.0f, 10.0f, 30.0f, 30.0f)
...
-(id)initWithFrame:(CGRect)frame GetSubtitleBlock:(NSString *(^)(void))subtitle GetTotalCountBlock:(NSUInteger (^)(void))TotalCount GetCurrentIndexBlock:(NSUInteger (^)(void))CurrentIndex{
    ...
    if (self) {
        ...
        _countleLabel.text = [NSString stringWithFormat:@"%d/%d",(int)_getCurrentIndex(),(int)_getTotalCount()];
    }
    return self;
}
-(void)refreshView{
